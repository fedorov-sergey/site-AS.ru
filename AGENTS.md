...
8. Все ссылки на сторонние сайты (YouTube, VK, Дзен, Boosty и др.) открываются в **новой вкладке** (`target="_blank" rel="noopener"`). Переходы между страницами сайта — в **той же вкладке** (без `target`).
9. Папка `Exampl/` — для примеров-прототипов. Если подразумевается сделать несколько вариантов страницы (или блока) и потом выбрать лучший, размещать их в `Exampl/` с понятными названиями (например, `variant_row.html`, `variant_carousel.html`). После выбора — переносить нужный вариант в основной каталог сайта.
10. Папка `FavBlocks/` — избранные (понравившиеся) блоки. Каждый блок — отдельный HTML-файл (только секция `<section>`, без навигации/футера) + txt-файл с описанием (терминологией для запросов). При добавлении нового блока — всегда создавать оба файла.
